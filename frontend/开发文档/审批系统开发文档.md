...
export const approvalRequest = sqliteTable('approval_request', {
  ...
  data: text('data'), // JSON业务数据（用于存储额外的业务信息）
...
export const member = sqliteTable('member', {
  id: integer('id').primaryKey({ autoIncrement: true }),
  userOrgRoleId: integer('user_org_role_id').notNull().references(() => userOrganizationRoleMap.id),
  memberNo: text('member_no').notNull().unique(),
  memberType: text('member_type').notNull(), // individual/corporate/student
  memberLevel: text('member_level').notNull(), // regular/premium/vip
  memberStatus: text('member_status').notNull().default('pending_approval'), // 包含审批状态
  joinDate: integer('join_date', { mode: 'timestamp' }),
  expireDate: integer('expire_date', { mode: 'timestamp' }),
  contactInfo: text('contact_info'), // JSON
...
#### 1. approvalFlow.flowConfig 字段结构
...
interface ApprovalStep {
  stepOrder: number;           // 步骤顺序，从1开始
  stepName: string;            // 步骤名称，如"秘书处审核"
  approverType: 'role' | 'user' | 'supervisor' | 'dynamic'; // 审批人类型
  approverConfig?: {           // 审批人配置（根据approverType决定）
    roleId?: string;           // 角色ID（当approverType为'role'时）
    userId?: number;           // 用户ID（当approverType为'user'时）
  };
  timeoutHours?: number;       // 超时时间（小时）
  condition?: {                // 条件判断（当approverType为'dynamic'时）
...
#### 2. approvalRequest.data 字段结构

```typescript
interface BusinessData {
  // 额外的业务信息（主要业务数据存储在实体表中）
  additionalInfo?: any;         // 额外的业务信息
...
| 类型 | 说明 | 配置示例 |
|------|------|----------|
| `role` | 基于角色，该角色的所有用户都可以审批 | `{ roleId: 'secretary' }` |
| `user` | 基于具体用户，只有指定用户可以审批 | `{ userId: 123 }` |
| `supervisor` | 基于上级，申请人的直接上级审批 | 无需配置 |
| `dynamic` | 基于条件动态决定，根据业务数据判断 | `{ condition: { field: 'amount', operator: '>', value: 5000 } }` |

#### 4. 条件判断说明

```typescript
interface Condition {
  field: string;               // 字段名，对应data中的字段
  operator: '>' | '<' | '>=' | '<=' | '==' | '!='; // 操作符
  value: number | string;      // 比较值
}
```
...
export class ApprovalService {
  ...
  async submitMembershipApplication(data: {
    ...
    // 1. 直接创建会员记录（状态为待审批）
    const member = await db.insert(member).values({
      userOrgRoleId: data.userOrgRoleId,
      memberNo: this.generateMemberNo(),
      memberType: data.memberType,
      memberLevel: data.memberLevel,
      memberStatus: 'pending_approval', // ✅ 直接标注为待审批
    ...
    const flow = await db.query.approvalFlow.findFirst({
      where: eq(approvalFlow.code, 'membership_approval')
    });
    
    if (!flow) {
      throw new Error('会员审批流程不存在');
    }
    ...
    const request = await db.insert(approvalRequest).values({
      ...
      title: '会员申请',
      content: '申请加入协会',
      data: data.additionalInfo ? JSON.stringify(data.additionalInfo) : null,
      memberId: member[0].id, // ✅ 直接关联会员记录
      status: 'pending',
      currentStep: 1
    }).returning();
    ...
    await db.update(member)
      .set({ approvalRequestId: request[0].id })
      .where(eq(member.id, member[0].id));
    ...
    return { member: member[0], request: request[0] };
  }
    ...
    if (data.action === 'reject') {
      ...
      await Promise.all([
        ...
        db.update(member)
          ...
          .where(eq(member.id, request.memberId))
      ]);
    } else {
      // 确定下一步审批人
  ...
  // 获取用户的申请记录（方案二：直接查询会员表）
  async getUserApplications(userId: number) {
    // ✅ 直接查询会员表，无需复杂关联
    const applications = await db.query.member.findMany({
  ...
  async getPendingMembers() {
    // ✅ 直接查询会员表，状态为待审批
    const pendingMembers = await db.query.member.findMany({
      where: eq(member.memberStatus, 'pending_approval'),
      with: {
        userOrganizationRoleMap: {
          with: {
            user: true,
  ...
  // 获取会员统计信息（方案二：简单统计）
  async getMemberStatistics(organizationId: number) {
    const stats = await db.select({
      total: sql`count(*)`,
      pending: sql`count(case when member_status = 'pending_approval' then 1 end)`,
      active: sql`count(case when member_status = 'active' then 1 end)`,
      rejected: sql`count(case when member_status = 'rejected' then 1 end)`,
      suspended: sql`count(case when member_status = 'suspended' then 1 end)`
    }).from(member)
    .innerJoin(userOrganizationRoleMap, eq(member.userOrgRoleId, userOrganizationRoleMap.id))
    .where(eq(userOrganizationRoleMap.organizationId, organizationId));
...
### 1. 开发简单性
- ✅ **数据流转简单**：不需要处理JSON序列化/反序列化
- ✅ **查询逻辑简单**：直接查询业务表即可
- ✅ **状态管理简单**：状态直接在业务表中
- ✅ **错误处理简单**：依赖数据库约束，减少代码错误

### 2. 用户体验
- ✅ **状态显示清晰**：用户可以直接看到申请状态
- ✅ **查询响应快速**：无需复杂关联查询
- ✅ **操作逻辑直观**：申请、审批、状态变更一目了然

### 3. 维护成本
- ✅ **代码逻辑清晰**：易于理解和修改
- ✅ **测试覆盖简单**：逻辑清晰，测试容易
- ✅ **扩展性好**：可以轻松添加新的业务字段和状态

### 4. 性能优势
- ✅ **查询效率高**：直接查询业务表，无需复杂关联
- ✅ **索引优化简单**：业务表索引设计清晰
- ✅ **缓存友好**：业务数据可以直接缓存
